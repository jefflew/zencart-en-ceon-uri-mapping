 ...
 * @version     $Id: class.CeonURIMappingInstallOrUpgrade.php 2018-06-17 08:20:05Z webchills $
 */
...
class CeonURIMappingInstallOrUpgrade
{
	 ...
	 * @access  protected
	 */
	public $_actions_performed = false;
	 ...
	 * @access  protected
	 */
	public $_uri_mappings_table_created = false;
	 ...
	 * @access  protected
	 */
	public $_uri_mapping_configs_table_created = false;
	 ...
	 * @access  protected
	 */
	public $_uri_mapping_prp_uri_parts_table_created = false;
	 ...
	 */
	function __construct($version, $installed_version)
	{
		global $db;
	 ...
	 */
	public function _checkCreateDatabases()
	{
	 ...
	 * @return  none
	 */
	public function _checkProductRelatedPagesURIPartsExistForLanguages()
	{
		global $db;
		...
		for ($i = 0, $n = count($languages); $i < $n; $i++) {
			...
			foreach ($page_types as $page_type) {
				if (!isset($current_uri_parts[$language_code][$page_type]) ||
						...
						(null === $current_uri_parts[$language_code][$page_type] ||
		...
		for ($i = 0, $n = count($languages); $i < $n; $i++) {
			...
			if (!isset($current_uri_parts[$language_code])) {
				...
				foreach ($page_types as $page_type) {
					if (null === $current_uri_parts[$language_code][$page_type] ||
							strlen($current_uri_parts[$language_code][$page_type]) == 0) {
	 ...
	 */
	public function _ensureDatabaseIsUpToDate()
	{
		global $db;
		...
		if (!$this->_uri_mappings_table_created && !$this->_checkUpdateURIMappingsTable()) {
            $problem_updating_table = true;
        }
		
		if (!$this->_uri_mapping_configs_table_created && !$this->_checkUpdateConfigsTable()) {
            $problem_updating_table = true;
        }
	 ...
	 */
	public function _checkUpdateURIMappingsTable()
	{
		global $db, $messageStack;
	 ...
	 * @return  none
	 */
	public function _migrateVersion2URIMappings()
	{
		global $db, $languages;
		...
		while (!$uri_mappings_result->EOF) {
			...
			if (null !== $uri_mappings_result->fields['category_id']) {
			...
			} else if (null !== $uri_mappings_result->fields['product_id']) {
			...
			} else if (null !== $uri_mappings_result->fields['page_id']) {
			...
			if (null !== $uri_mappings_result->fields['product_id']) {
	 ...
	 * @return  none
	 */
	public function _migrateFromOldAlternateURIMappingFormat()
	{
		global $db;
		...
		while (!$uri_mappings_result->EOF) {
			...
			if (preg_match($pattern, $uri_mappings_result->fields['alternate_uri'], $matches)) {
				...
				$update_record_format_query = "
					...
					SET
						main_page = '" . zen_db_input($main_page) . "',
						query_string_parameters = " . (null === $parameter_string ? 'null' :
	 ...
	 * @return  boolean   Whether or not the database table is up to date.
	 */
	public function _checkUpdateConfigsTable()
	{
		global $db, $messageStack;
	 ...
	 * @return  none
	 */
	public function _checkForDefaultConfig()
	{
		global $db;
	 ...
	 * @return  none
	 */
	public function _changeDefaultRemoveWordsSetting()
	{
		global $db;
		...
		for ($i = 0, $iMax = count($remove_words); $i < $iMax; $i++) {
	 ...
	 */
	public function _checkZenCartConfigGroupAndOption()
	{
		global $db, $messageStack;
		...
		if (function_exists('zen_register_admin_page') && !zen_page_key_exists('ceon_uri_mapping_config_group')) {
	 ...
	 * @return  none
	 */
	public function _makeIndexPageMappingsHistorical()
	{
		global $db;
	 ...
	 * @return  none
	 */
	public function _stripTrailingSlashesFromMappings()
	{
		global $db;
	 ...
	 */
	public function _DBUserHasPrivilege($privilege_type)
	{
		global $db;
